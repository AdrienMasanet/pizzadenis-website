...
export const PizzaListContext = createContext<{ allPizzaList: Pizza[]; filteredPizzaList: Pizza[]; pizzaListFilters: PizzaListFilters }>({
  allPizzaList: [],
  filteredPizzaList: [],
  pizzaListFilters: {
    name: "",
...
export const PizzaListProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [filteredPizzaList, setFilteredPizzaList] = useState<Pizza[]>(allPizzaList);

  // Filter all existing pizzas based on the current filters when they change
  useEffect(() => {
    setFilteredPizzaList(
      allPizzaList.filter((pizza) => {
        if (pizzaListFilters.name && !pizza.name.toLowerCase().includes(pizzaListFilters.name.toLowerCase())) {
          return false;
        }
        if (pizzaListFilters.base && pizzaListFilters.base != "all" && !pizza.base.name.toLowerCase().includes(pizzaListFilters.base.toLowerCase())) {
          return false;
        }
        if (pizzaListFilters.vegetarianOnly && !pizza.ingredients.every((ingredient) => !ingredient.isMeat && !ingredient.isSeafood)) {
          return false;
        }
        if (pizzaListFilters.containsMeat && !pizza.ingredients.some((ingredient) => ingredient.isMeat)) {
          return false;
        }
        if (pizzaListFilters.containsSeaFood && !pizza.ingredients.some((ingredient) => ingredient.isSeafood)) {
          return false;
        }
        return true;
      })
